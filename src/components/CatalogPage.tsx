  ...
  return (
    <div className="page-stack catalog-page">
      ...
      <section className="catalog-summary-grid">
        ...
        <div className="catalog-group-strip">{groupCounts.map(({ group, count }) => <button key={group} className={activeGroup === group ? 'is-selected' : ''} onClick={() => onGroupChange(activeGroup === group ? 'all' : group)}><span style={{ backgroundColor: groupMeta[group].color }}>{group}</span><strong>{count}</strong></button>)}</div>
      </section>

      <section className="panel catalog-panel">
        <div className="panel-heading catalog-heading"><div><span className="panel-eyebrow">DICTIONARY INDEX</span><h3>ตัวชี้วัดตามรหัส THIP</h3><p>กดรายการใดก็ได้เพื่อดูโครงสร้าง detail รายเดือน; รายการที่ยังไม่ผูกข้อมูลจะแสดงสถานะ no-data อย่างตรงไปตรงมา</p></div><div className="catalog-controls"><label className="search-field catalog-search"><Search size={16} /><input value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder="ค้นหารหัสหรือชื่อ KPI" /></label><button className="catalog-all-button" onClick={() => onGroupChange('all')}>ทั้งหมด <ArrowUpRight size={14} /></button></div></div>
        <div className="catalog-table-wrap">
          <table className="catalog-table">
            <thead><tr><th>รหัส</th><th>ชื่อตัวชี้วัดจาก dictionary</th><th>กลุ่ม</th><th>สถานะข้อมูล</th><th aria-label="เปิดรายละเอียด" /></tr></thead>
            <tbody>{filtered.map((entry) => { const wired = wiredCodes.has(entry.code); return <tr key={entry.code} tabIndex={0} onClick={() => onOpen(entry.code)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') onOpen(entry.code); }}><td><strong className="catalog-code">{entry.code}</strong></td><td><span className="catalog-title">{entry.title}</span></td><td><span className="catalog-group"><i style={{ backgroundColor: groupMeta[entry.group].color }}>{entry.group}</i>{groupMeta[entry.group].shortLabel}</span></td><td><span className={`catalog-state ${wired ? 'catalog-state-wired' : 'catalog-state-pending'}`}><span />{wired ? 'มี data contract' : 'รอผูก source view'}</span></td><td><span className="catalog-arrow">↗</span></td></tr>; })}</tbody>
          </table>

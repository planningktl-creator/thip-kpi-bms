...
`tier` is `registered` for a code with an approved read-only query and `pending-local-source` for a code that is in the 232-code contract but still needs a hospital rule/code-set. A `pending-local-source` row must carry `denominator = NULL`, `value = NULL`, and a non-empty `pending_reason`; it is an explicit unavailable result, never a fabricated zero. The coverage summary separates `availableCellCount` (measured denominator) from `unavailableCellCount` (explicit NULL denominator), while `complete` still requires all 1,552 cadence-aware cells to be present. The reporting-layer DDL and per-fiscal-year refresh that produce these rows are generated by `pnpm sourceview:build` into `reporting/thip_kpi_monthly.sql`.
